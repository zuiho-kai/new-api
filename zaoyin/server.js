...
async function runImageRequest({ imagePath, body, authHeader, localIds = [] }) {
  ...
  const requestBody = imagePath === '/v1/images/edits'
    ? normalizeImageEditBody(body || {})
    : (body || {});
  const upstreamResp = await httpPostJson(
    upstream,
    requestBody,
...
function normalizeImageEditBody(body) {
  const sources = [];
  const addSource = (value) => {
    ...
    if (value && typeof value === 'object') {
      addSource(value.image_url);
    }
  };
  const addValues = (value) => {
    if (Array.isArray(value)) value.forEach(addSource);
    else addSource(value);
  };

  addValues(body.image);
  addValues(body.images);
  if (!sources.length) return { ...body };

  return {
    ...body,
    image: sources[0],
    images: sources.map(imageUrl => ({ image_url: imageUrl })),
  };
}
